...
@register_tool(
    "place_oco_order",
    description="Place an OCO (One-Cancels-the-Other) order via Interactive Brokers TWS API.",
    args_model=PlaceOcoOrderArgs,
    review=False,
)
async def place_oco_order(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async with ibTools.guarded():
        ...
        status, error = await wait_order_confirmed(tp_trade)
