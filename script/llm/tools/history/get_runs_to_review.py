...
@register_tool("get_runs_to_review", description="Retrieve runs that have been executed since the last review.", args_model=None, run=False)
async def get_runs_to_review(_: Dict[str, Any]) -> Dict[str, Any]:
    dbTools = DBTools.get_instance()
    
    last_review = dbTools.get_filtered_runs(trigger_type="review", status="completed", limit=1)
